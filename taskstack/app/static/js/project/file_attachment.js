const FileAttachment = {
    ...
    */
    ...
    uploadBoxAddFile: function(fileUploadBox_id, file_name, file_data) {  // only used programmatically
        ...
        fileUploadBox_files.innerHTML += '<div data-name="' + he.escape(file_name) + '" class="file" onclick="FileAttachment._uploadBoxShowRemoveFileIcon(event);">' + 
    ...
    _processUploadBoxDropInput: async function(e) {
        e.preventDefault();
        ...
        for (let file of e.dataTransfer.files) {
            if (file.size > 8388608) {
                ...
                fileUploadBox_files.innerHTML += '<div data-name="' + he.escape(file.name) + '" class="file" onclick="FileAttachment._uploadBoxShowRemoveFileIcon(event);">' + 
                '<i class="fas fa-window-close cancelIcon" onclick="FileAttachment.uploadBoxRemoveFile(event);"></i>' +
                '<i class="fas fa-file fileIcon"></i>' +
    ...
    _processUploadBoxInput: async function(e) {
    ...
    _uploadBoxShowRemoveFileIcon: function(e) {
        var file = e.target;
    ...
    _uploadBoxDragOverHandler: function(e) {
        e.preventDefault();
    ...
    getHTML: function(file_name, file_url) {
        return '<a data-name="'+ he.escape(file_name) +'" class="file" href="'+ file_url +'" download>'+
        '<i class="fas fa-file fileIcon"></i>'+
        '<p>'+ he.escape(file_name) +'</p>'+
        '</a>';
    },
